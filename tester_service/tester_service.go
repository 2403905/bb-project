...
func main() {
	// backendApi added to enable service containerization
	var backendApi = flag.String("service-api", "localhost:9090", "backend API address")
	flag.Parse()
	...
	go func() {
		...
		for {
			...
			resp, err := client.Post("http://"+*backendApi+"/callback", "application/json", body)
			if err != nil {
				fmt.Println(err)
				continue
			}
